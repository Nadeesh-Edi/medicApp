...
const EditAppointment = ({ navigation, route }) => {
  const [aptNo, setAptNo] = useState(route.params.aptNo);
  ...
  const getAppointmentDetails = () => {
    axios.get(`${API_KEY}/patientRoutes/getAppointmentById/${aptNo}`).then((res) => {
        const apptment = res.data;

        setName(apptment.patient.name)
        setAge(apptment.patient.age)
        setPhone(apptment.patient.contactNo)
        setEmail(apptment.patient.email)
        setSelectedDoc(apptment.doctor)
        setAptId(apptment._id)
    }).catch((err) => {
        console.log(err);
        openErrorPopup()
    })
  ...
  const editAptment = () => {
    ...
    axios
      .put(`${API_KEY}/patientRoutes/editAppointment`, passData)
      .then((res) => {
        openSuccessPopup();
      })
      .catch((err) => {
      ...
      <ScrollView>
        ...
        <View style={styles.containerBox}>
          <SelectDropdown
            ...
            defaultButtonText={selectedDoc.name}
